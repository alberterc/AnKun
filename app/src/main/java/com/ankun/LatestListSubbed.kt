...
class LatestListSubbed : AppCompatActivity() {
    ...
    private fun getAnimeRecentUpdatesList(pageNumber: String) {
        animeScrape.GetLatestList(pageNumber, "sub").execute()
    ...
    private fun initTopToolbar() {
        ...
        topToolbar.setOnMenuItemClickListener {
            when (it.itemId) {
                R.id.dub_release -> {
                    startActivity(Intent(applicationContext, LatestListDubbed::class.java))
                    finish()
                }
            }
            true
